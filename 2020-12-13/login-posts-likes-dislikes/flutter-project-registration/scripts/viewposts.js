((this_) => {
	const db = firebase.database();
	const tweetsDB = db.ref('/tweets');
	const postContainer = document.getElementById('post-container');
	const newPost = document.getElementById('new-post');
	const loader = document.getElementById('loader');

	const post = data => {
		const state = data.val();

		return `<figure class="profile-avatar post-section-avatar">
              <img src="images/avatar.png" alt="BP" class="profile-image">
          </figure>
          <div class="post-content">
              <div class="post-author">
                  <h3 class="post-author-name">${state.username}</h3>
                  <span class="post-delimiter fa fa-circle"></span>
                  <span class="post-date">${time_ago(new Date(state.date))}</span>
              </div>
              <p class="post-text">${state.message}</p>
              <div class="post-footer">
                  <div class="post-reaction">
                      <button class="far fa-thumbs-up like-btn" data-id="${data.key}"></button>
                      <span class="post-likes">${state.likes}</span>
                  </div>

                  <div class="post-reaction dislike-container">
                      <button class="far fa-thumbs-down dislike-btn" data-id="${data.key}"></button>
                      <span class="post-dislikes">${state.dislikes}</span>
                  </div>
              </div>
          </div>
          <div class="post-close">
              <button class="post-close fa fa-times" data-id="${data.key}"></button>
          </div>`;
	};

	// Logic when posting new tweet
	newPost.addEventListener('submit', event => {
		event.preventDefault();
		tweet.post(event.target[0].value);

		document.getElementById('profile-posts-count').innerText++;
	});


	// Update profile posts information
	firebase.auth().onAuthStateChanged(user => {
		document.getElementById('profile-name').innerText = user.displayName;

		auth.getUserStats(user.uid).on('value', (snapshot) => {
			const data = snapshot.val();

			document.getElementById('profile-posts-count').innerText = data ? data.tweets : 0;
			document.getElementById('profile-likes-count').innerText = data ? data.likes : 0;
		});
	});

	// Logic when new tweet is added
	tweetsDB.on('child_added', data => {
		if (loader) {
			loader.remove();
		}

		const newTweet = document.createElement('div');
		newTweet.innerHTML = post(data);
		newTweet.id = data.key;

		const likeBtn = newTweet.querySelector('.like-btn');
		likeBtn.clicked = false;
		const dislikeBtn = newTweet.querySelector('.dislike-btn');
		dislikeBtn.clicked = false;
		const closeBtn = newTweet.querySelector('.post-close')

		likeBtn.addEventListener('click', e => {
			if (!validateUser()) {
				return;
			}
			const id = e.target.getAttribute('data-id');

			if (e.target.clicked) {
				window.tweet.decrementLikes(id);
				likeBtn.clicked = false;
			} else {
				window.tweet.incrementLikes(id);
				
				likeBtn.clicked = true;
				if (dislikeBtn.clicked) {
					window.tweet.decrementDislikes(id);
					dislikeBtn.clicked = false;
				}
			}
		});
		dislikeBtn.addEventListener('click', e => {
			if (!validateUser()) {
				return;
			}
			const id = e.target.getAttribute('data-id');

			if (e.target.clicked) {
				window.tweet.decrementDislikes(id);
				dislikeBtn.clicked = false;
			} else {
				window.tweet.incrementDislikes(id);
				
				dislikeBtn.clicked = true;
				if (likeBtn.clicked) {
					window.tweet.decrementLikes(id);
					likeBtn.clicked = false;
				}
			}
		});

		closeBtn.addEventListener('click', e => {
			const id = e.target.getAttribute('data-id');

			if (data.val().userId == firebase.auth().currentUser.uid) {
				tweet.delete(id);

				newTweet.remove();
			}
		});

		postContainer.insertBefore(newTweet, postContainer.firstChild);
	});

	function reactionClickHandler(e, callback) {
		const id = e.target.getAttribute('data-id');

		callback(id);
	}

	// Like / Dislike logic here
	tweetsDB.on('child_changed', data => {
		let stats = data.val();
		let tweet = document.getElementById(data.key);


		tweet.querySelector('.post-likes').innerText = stats.likes;
		tweet.querySelector('.post-dislikes').innerText = stats.dislikes;
	});

	function validateUser() {
		if (!firebase.auth().currentUser) {
			// user is not logged in
			window.location = 'index.html?error=accessDenied';
			return false;
		}

		return true;
	}

	// Helper function for converting time from milliseconds to human readable format
	function time_ago(time) {
		const time_formats = [
			[60, 'seconds', 1], // 60
			[120, '1 minute ago', '1 minute from now'], // 60*2
			[3600, 'minutes', 60], // 60*60, 60
			[7200, '1 hour ago', '1 hour from now'], // 60*60*2
			[86400, 'hours', 3600], // 60*60*24, 60*60
			[172800, 'Yesterday', 'Tomorrow'], // 60*60*24*2
			[604800, 'days', 86400], // 60*60*24*7, 60*60*24
			[1209600, 'Last week', 'Next week'], // 60*60*24*7*4*2
			[2419200, 'weeks', 604800], // 60*60*24*7*4, 60*60*24*7
			[4838400, 'Last month', 'Next month'], // 60*60*24*7*4*2
			[29030400, 'months', 2419200], // 60*60*24*7*4*12, 60*60*24*7*4
			[58060800, 'Last year', 'Next year'], // 60*60*24*7*4*12*2
			[2903040000, 'years', 29030400], // 60*60*24*7*4*12*100, 60*60*24*7*4*12
			[5806080000, 'Last century', 'Next century'], // 60*60*24*7*4*12*100*2
			[58060800000, 'centuries', 2903040000] // 60*60*24*7*4*12*100*20, 60*60*24*7*4*12*100
		];
		let seconds = (+new Date() - time) / 1000,
			token = 'ago',
			list_choice = 1;

		if (seconds === 0) {
			return 'Just now'
		}
		if (seconds < 0) {
			seconds = Math.abs(seconds);
			token = 'from now';
			list_choice = 2;
		}
		let i = 0,
			format;
		while (format = time_formats[i++])
			if (seconds < format[0]) {
				if (typeof format[2] === 'string')
					return format[list_choice];
				else
					return Math.floor(seconds / format[2]) + ' ' + format[1] + ' ' + token;
			}
		return time;
	}
})(this);